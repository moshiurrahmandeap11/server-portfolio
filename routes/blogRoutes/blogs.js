...
const BLOGS_LIST_CACHE_PREFIX = "blogs:list";
...
router.get("/", async (req, res) => {
  try {
    ...
    const cacheKey = `${BLOGS_LIST_CACHE_PREFIX}:page:${page}:limit:${limit}`;
    ...
    if (cached) {
      return res.status(200).json({
        success: true,
        message: "All blogs successfully fetched (cached)",
        ...cached,
      });
    }
    ...
    // Store in cache
    await setCache(cacheKey, responseData, DEFAULT_CACHE_TTL);
